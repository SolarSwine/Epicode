...
export class PostService {
  ...
  getPostById(id: number): Observable<PostInterface | undefined> {
    const post = this.posts.find(post => post.id === id);
    return of(post);
  }

  getTopPost(): Observable<PostInterface> {
    const topPost = this.posts.sort((a, b) => b.reactions - a.reactions)[0];
    return of(topPost);
  }

  getRandomPosts(num: number): Observable<PostInterface[]> {
    ...
    return of(randomPosts);
  }

  filterPostsByTag(tag: string): Observable<PostInterface[]> {
